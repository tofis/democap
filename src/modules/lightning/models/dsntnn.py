...
def soft_argmax_3d(heatmaps, depth_dim_scale=3):
    assert isinstance(heatmaps, torch.Tensor)
    out_divider = 4
    num_of_points = heatmaps.size()[1] // depth_dim_scale
    out_size = [out_divider * out_divider * depth_dim_scale, heatmaps.size()[2] // out_divider, heatmaps.size()[3] // out_divider]

    heatmaps = heatmaps.reshape((-1, num_of_points, out_divider * out_divider * depth_dim_scale * out_size[1] * out_size[2]))
    heatmaps = F.softmax(heatmaps, 2)
    heatmaps = heatmaps.reshape((-1, num_of_points, out_size[0], out_size[1], out_size[2]))

    accu_x = heatmaps.sum(dim=(2,3))
    accu_y = heatmaps.sum(dim=(2,4))
    accu_z = heatmaps.sum(dim=(3,4))

    accu_x = accu_x * torch.cuda.comm.broadcast(torch.arange(1,out_size[2]+1).type(torch.cuda.FloatTensor), devices=[accu_x.device.index])[0]
    accu_y = accu_y * torch.cuda.comm.broadcast(torch.arange(1,out_size[1]+1).type(torch.cuda.FloatTensor), devices=[accu_y.device.index])[0]
    accu_z = accu_z * torch.cuda.comm.broadcast(torch.arange(1,out_size[0]+1).type(torch.cuda.FloatTensor), devices=[accu_z.device.index])[0]
...
def flat_softmax(inp):
    ...
    flat = inp.reshape(-1, reduce(mul, orig_size[2:]))
...
def squared_losses(actual, target):
    """Calculate the Euclidean losses for multi-point samples.
    ...
    """
    assert actual.size() == target.size(), 'input tensors must have the same size'
    e = torch.norm(actual - target, p=2, dim=-1, keepdim=False)
    return e * e
